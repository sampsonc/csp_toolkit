...
class TestAIViolationEnhancement:
    ...
    def test_ai_enhance_no_anthropic(self, mock_has_anthropic):
        ...
        violations = [
            {"blocked-uri": "https://example.com/script.js", "effective-directive": "script-src", "violated-directive": "script-src"}
        ]
    ...
    def test_ai_enhance_success(self, mock_anthropic_class, mock_has_anthropic):
        ...
        mock_response.content = json.dumps({
            ...
            "implementation_notes": ["Add script sources to script-src directive", "Test thoroughly before deploying"],
            "risk_assessment": "medium"
        })
        mock_client.messages.create.return_value = mock_response

        violations = [
            {"blocked-uri": "https://cdn.example.com/script.js", "effective-directive": "script-src", "violated-directive": "script-src"}
        ]
        ...
        assert result["ai_analysis"]["explanation"] == "These violations occur because external scripts are blocked"
    ...
    def test_build_violation_analysis_prompt(self):
        ...
        summary = {
            "count": 5,
            "groups": [
                {"blocked_uri": "https://cdn.example.com/script.js", "effective_directive": "script-src", "violated_directive": "script-src", "count": 3},
                {"blocked_uri": "https://fonts.googleapis.com/css", "effective_directive": "style-src", "violated_directive": "style-src", "count": 2}
            ]
        }

        policy = parse("default-src 'self'")
        suggestions = [
            {"directive": "script-src", "suggested_source": "https://cdn.example.com", "action": "consider_adding_source"}
        ]
    ...
    def test_ai_enhance_api_error(self, mock_anthropic_class, mock_has_anthropic):
        ...
        violations = [{"blocked-uri": "test.com", "effective-directive": "script-src", "violated-directive": "script-src"}]
    ...
    def test_ai_enhance_malformed_json(self, mock_anthropic_class, mock_has_anthropic):
        ...
        mock_response = Mock()
        mock_response.content = "This is not valid JSON but has useful explanation about CSP violations"
        mock_client.messages.create.return_value = mock_response
        ...
        assert result["ai_analysis"]["explanation"] == "This is not valid JSON but has useful explanation about CSP violations"